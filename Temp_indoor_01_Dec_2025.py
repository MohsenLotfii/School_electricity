    ...
    for fp in paths:
        try:
            ...
            if ts_col is None or temp_col is None:
                print(f"[SKIP] {os.path.basename(fp)}: couldn’t identify timestamp/temperature columns.")
                continue
...
def plot_representative_day(profile, school, season_name):
    ...
    plt.title(f"{school} — Representative {season_name} Day Temperature Profile")
...
def estimate_setpoints(rep_profile, season_name, school, day_hours=range(6, 18)):
    ...
    day_mask = df["Hour"].isin(day_hours)   # Day = 06:00–17:59
...
def print_setpoint_summary(school, summer_sp, winter_sp):
    ...
    if summer_sp:
        print("🌞 SUMMER:")
        print(f"  Day   Setpoint ≈ {summer_sp['Day_Setpoint']:.2f} °C")
        print(f"  Night Setpoint ≈ {summer_sp['Night_Setpoint']:.2f} °C")
    else:
        print("🌞 SUMMER: No data")

    if winter_sp:
        print("❄️ WINTER:")
        print(f"  Day   Setpoint ≈ {winter_sp['Day_Setpoint']:.2f} °C")
        print(f"  Night Setpoint ≈ {winter_sp['Night_Setpoint']:.2f} °C")
    else:
        print("❄️ WINTER: No data")
...
def plot_setpoint_profiles(school, summer_sp, winter_sp):
    ...
    plt.title(f"{school} – Daily HVAC Setpoint Profile (Summer vs Winter)")
    plt.xlabel("Hour of Day")
    plt.ylabel("Temperature (°C)")
    plt.xticks(range(0, 24, 2))
    plt.grid(True, alpha=0.3)
...
def plot_raw_timeseries(df_raw, school):
    ...
    plt.title(f"{school} — Raw Temperature vs Time")
    plt.xlabel("Time")
    plt.ylabel("Temperature (°C)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_typical_week_line(pivot, school, freq_label=FREQ):
    """
    Typical week per-day curves + dashed 'mean weekdays' (Mon–Fri).
    """
    if pivot.empty:
    ...
    plt.title(f"{school} — Typical Week Temperature Profile ({freq_label})")
...
def plot_typical_week_heatmap(pivot, school):
    ...
    plt.title(f"{school} — Typical Week Temperature (Heatmap)")
    plt.yticks(
        ticks=range(7),
...
# ============================================================
#  WEATHER DATA MODULE – Mirabel hourly file
# ============================================================
...
def plot_weather_representative(profile, season_name):
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(profile["Hour"], profile["Temperature"], linewidth=2)
    plt.title(f"Weather – Representative {season_name} Day (Mirabel)")
    plt.xlabel("Hour of Day")
    plt.ylabel("Temperature (°C)")
    plt.xticks(range(0, 24, 2))
    plt.grid(True, alpha=0.3)
...
def build_typical_weeks_for_all_schools(
    ...
    for school in schools:
        ...
        if mode == "folder":
            ...
            print(f"🔍 {school}: folder match -> {folder} ({len(csvs)} CSVs)")
            raw = read_school_temperature_series_from_folder(folder)
        else:
            files = resource
            print(f"🔍 {school}: file-pattern match -> {len(files)} CSVs")
    ...
    if os.path.isfile(weather_path):
        ...
        plt.title("Mirabel Weather – Raw Hourly Temperature")
        plt.xlabel("Time")
        plt.ylabel("Temperature (°C)")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
...
# ============================================================
#  BUILD FINAL MERGED DATAFRAME (WINTER & SUMMER)
# ============================================================
# === Add internal gain calculation to build_season_dataframe ===
def q_internal(hour):
    if 8 <= hour < 12:
        return 800  # morning classes
    elif 12 <= hour < 13:
        return 200  # lunch break
    elif 13 <= hour < 17:
        return 800  # afternoon classes
    else:
        return 0  # unoccupied

def q_lighting(hour):
    return 10 if 7 <= hour < 18 else 0

def q_equipment(hour):
    return 5 if 8 <= hour < 17 else 0

# Define student and area info per school
SCHOOL_OCCUPANCY = {
    "Jeunes Du Mondes": {"students": 701, "area": 3598},
    "École de l’Aquarelle": {"students": 381, "area": 7949.8},
}

# Modify build_season_dataframe to include Internal_Gain_Wm2
def build_season_dataframe(school_name, results, season="winter"):
    """
    season = "winter" or "summer"
    Returns a dataframe with:
        Timestamp | T_outdoor | T_setpoint | T_indoor | Internal_Gain_Wm2 | Lighting_Gain_Wm2 | Equipment_Gain_Wm2 | Internal_Gain_Total_Wm2
    """
    school = results.get(school_name)
    if school is None:
        print(f"[ERROR] No results found for {school_name}")
        return pd.DataFrame()

    indoor = school["regularized"].copy()
    indoor = indoor.rename(columns={"Temperature": "T_indoor"})
    indoor["Timestamp"] = pd.to_datetime(indoor["Timestamp"])

    wx = results["WEATHER"]["regularized"].copy()
    wx = wx.rename(columns={"Temperature": "T_outdoor"})
    wx["Timestamp"] = pd.to_datetime(wx["Timestamp"])

    if season.lower() == "winter":
        sp = school["winter_setpoints"]
        months = [12, 1, 2]
    else:
        sp = school["summer_setpoints"]
        months = [6, 7, 8]

    if sp is None:
        print(f"[ERROR] No setpoints available for {school_name} in {season}")
        return pd.DataFrame()

    day_sp = round(sp["Day_Setpoint"], 1)
    night_sp = round(sp["Night_Setpoint"], 1)

    df = pd.merge_asof(
        indoor.sort_values("Timestamp"),
    ...
    df["Hour"] = df["Timestamp"].dt.hour
    df["T_setpoint"] = df["Hour"].apply(lambda h: day_sp if 6 <= h < 18 else night_sp)

    # Add Internal Gain per m2 based on school occupancy
    school_meta = SCHOOL_OCCUPANCY.get(school_name)
    if school_meta:
        students = school_meta["students"]
        area = school_meta["area"]
        df["Internal_Gain_Wm2"] = df["Hour"].apply(
            lambda h: (q_internal(h) * students) / area
        )
    else:
        df["Internal_Gain_Wm2"] = 0

    df["Lighting_Gain_Wm2"] = df["Hour"].apply(q_lighting)
    df["Equipment_Gain_Wm2"] = df["Hour"].apply(q_equipment)

    df["Internal_Gain_Total_Wm2"] = (
        df["Internal_Gain_Wm2"] +
        df["Lighting_Gain_Wm2"] +
        df["Equipment_Gain_Wm2"]
    )

    df = df[[
        "Timestamp", "T_outdoor", "T_setpoint", "T_indoor",
        "Internal_Gain_Wm2", "Lighting_Gain_Wm2", "Equipment_Gain_Wm2",
        "Internal_Gain_Total_Wm2"
    ]]

    df[[
        "T_outdoor", "T_indoor", "T_setpoint",
        "Internal_Gain_Wm2", "Lighting_Gain_Wm2",
        "Equipment_Gain_Wm2", "Internal_Gain_Total_Wm2"
    ]] = df[[
        "T_outdoor", "T_indoor", "T_setpoint",
        "Internal_Gain_Wm2", "Lighting_Gain_Wm2",
        "Equipment_Gain_Wm2", "Internal_Gain_Total_Wm2"
...
# === Plot: Typical Daily Internal Gains ===
def plot_typical_day_internal_gains(df, season_label):
    # Compute average per hour
    df["Hour"] = pd.to_datetime(df["Timestamp"]).dt.hour
    hourly_avg = df.groupby("Hour")[[
        "Internal_Gain_Wm2", "Lighting_Gain_Wm2", "Equipment_Gain_Wm2", "Internal_Gain_Total_Wm2"
    ]].mean().reset_index()

    # Plot
    plt.figure(figsize=(10, 5))
    plt.plot(hourly_avg["Hour"], hourly_avg["Internal_Gain_Wm2"], label="Occupancy Gain", color="#8da0cb")
    plt.plot(hourly_avg["Hour"], hourly_avg["Lighting_Gain_Wm2"], label="Lighting Gain", color="#66c2a5")
    plt.plot(hourly_avg["Hour"], hourly_avg["Equipment_Gain_Wm2"], label="Equipment Gain", color="#fc8d62")
    plt.plot(hourly_avg["Hour"], hourly_avg["Internal_Gain_Total_Wm2"], label="Total Internal Gain", linestyle="--", color="#a6d854", linewidth=2)

    plt.title(f"Typical Daily Internal Gains — {season_label.capitalize()}")
    plt.xlabel("Hour of Day")
    plt.ylabel("Gain (W/m²)")
    plt.xticks(range(0, 24, 2))
    plt.grid(alpha=0.3)
...
if __name__ == "__main__":
    ...
    df_winter = build_season_dataframe("École de l’Aquarelle", results, season="winter")
    df_summer = build_season_dataframe("École de l’Aquarelle", results, season="summer")
    ...
    # Plot typical daily internal gains
    plot_typical_day_internal_gains(df_winter, "winter")
    plot_typical_day_internal_gains(df_summer, "summer")
